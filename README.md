...
> ステータス: **P11 完了（P7〜P11 含む / Render + Turso + Claude Haiku で本番稼働中）**
> URL: `https://koto-log.onrender.com`

## できること

- 自由文を解釈して `feeding / sleep / diaper / temp / baby_food / bath / medicine / hospital / outing / height / weight` を構造化保存
  （例: 「3時にミルク120ml」「さっき寝た」「うんちした」「離乳食50g」「お風呂入れた」「薬飲んだ」「身長60cm」「体重5.2kg」）
...
- **成長曲線**（`/dashboard/growth`、P11）：身長・体重の記録を月齢別に散布図で表示。厚労省標準値（P3/P50/P97）と重ねて比較。男女別・`children.sex` DB 管理対応
...
│   └── prompts.py      # prompts/<name>/<version>.txt 読込ローダ（E1-3）
├── prompts/
│   ├── extract/v1.txt  # extractor.py のシステムプロンプト（E1-3、バージョン管理）
...
`evals/golden/utterances.yaml`（85件）に対して、本番と同じプロンプト（`kotolog.agent.extractor` /
`kotolog.agent.loop`）とツール定義（`kotolog.tools.definitions.TOOLS`）で実 LLM 呼び出しを行い、
`evals/scoring.py` で採点する。実 LLM 呼び出しが発生するため、事前にローカル Ollama を起動するか
`KOTOLOG_API_KEY` を設定しておくこと（「ローカル開発セットアップ」参照）。

```bash
# ゴールデンセット全件を採点し、evals/results/ にJSON保存
uv run python -m evals.runner

# モデルを上書き（KOTOLOG_MODEL の代わりに）
uv run python -m evals.runner --model anthropic/claude-haiku-4-5-20251001

# ゴールデンセット・出力先を指定
uv run python -m evals.runner --golden evals/golden/utterances.yaml --out evals/results
```

結果 JSON（`evals/results/<timestamp>_<model>.json`）には以下を含む:

| フィールド | 内容 |
|---|---|
| `model` | 採点に使用したモデル文字列 |
| `prompt_versions` | 使用したプロンプトのバージョン（例: `{"extract": "v1", "loop": "v1"}`） |
...
| P10 記録項目拡充 | 5種別追加（離乳食・お風呂・薬・病院・外出）| ✅ 完了 |
| P11 成長記録・成長曲線 | 身長・体重記録・`children.sex` DB 管理・厚労省標準値・`/dashboard/growth` | ✅ 完了 |
| 今後 | P12 予防接種リマインド | — |
