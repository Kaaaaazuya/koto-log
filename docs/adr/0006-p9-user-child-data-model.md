...
本設計は規模が大きいため、**各段階が単独で動きテスト緑を保つ**形で分割実装する
（本 ADR は分割せずアンブレラ設計として維持）。

| 段階 | 内容 | 依存 | 挙動変化 |
|---|---|---|---|
| **P9.0** | マイグレーション基盤（`schema_migrations`＋`migrate()`＋baseline スタンプ） | なし | なし（インフラのみ） |
| **P9.1** | データモデル拡張（`users`・`default_child_id`・children 複数 CRUD・`KOTOLOG_DEFAULT_CHILD` 撤廃） | P9.0 | DB が複数子対応に |
| **P9.2** | 対象児解決＋executor リクエスト単位化 | P9.1 | 複数子の会話挙動 |
| **P9.3** | 共有（ユーザー管理 UI・自動登録・Push fan-out＝旧 ADR-0004） | P9.1 | 家族共有 |

**P9.0 を最初に単独で出す**: 機能ゼロ・挙動変化なしでレビュー／テストが独立し、以降の
スキーマ変更が安全に乗る（de-risk）。P9.2 と P9.3 は P9.1 の後なら並行可。

## 実装計画（詳細）

> 本 ADR は T9.1（設計）。以降の実装タスクは開発計画 P9.0〜P9.3 に対応。
