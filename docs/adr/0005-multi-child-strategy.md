...
- **日付:** 2026-06-26
- **状態:** 🚫 棄却（2026-06-28・[ロードマップ v2](../koto-log_roadmap_v2_refined.md) §2-A により方針転換）

> **棄却理由**: ロードマップ v2 の再優先度づけで、複数子は「**1アプリ内に複数子を持ち、
> 会話で対象児を解決**する（`KOTOLOG_DEFAULT_CHILD` 撤廃・user–child データモデルへ移行）」
> 方式に決定した。本 ADR の「子＝LINEアカウント固定（案A/案B）」とは両立しないため棄却する。
> 新方式は開発計画の P9 に再構成済みで、データモデル設計は別途新 ADR で起こす。
> 以下の本文は経緯の記録として残す。
