...
def test_csrf_token_is_unique_per_session(client):
    """CSRF トークンはセッション内で同じ（セッションベースの CSRF 保護）。"""
    import re
    ...
    # セッション内では同じトークンが再利用される
    assert token1 == token2, "CSRF tokens should be the same within a session"


def test_csrf_token_from_different_endpoint_accepted(client):
    """同一セッションのトークンは複数のエンドポイントで使用可能。"""
    import re
